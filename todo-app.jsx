...
let db = [
  { _id: "1", title: "Read the MERN stack docs", completed: true, priority: "low", createdAt: new Date(Date.now() - 86400000 * 2).toISOString() },
  { _id: "2", title: "Build a production-ready REST API", completed: false, priority: "high", createdAt: new Date(Date.now() - 86400000).toISOString() },
  { _id: "3", title: "Write unit tests with Jest", completed: false, priority: "medium", createdAt: new Date().toISOString() },
];
let idCounter = 4;
const api = {
  async getAll() { await delay(200); return [...db]; },
  async create(data) { await delay(150); const t = { _id: String(idCounter++), ...data, completed: false, createdAt: new Date().toISOString() }; db.push(t); return t; },
  async update(id, patch) { await delay(150); const i = db.findIndex(t => t._id === id); if (i < 0) throw new Error("Not found"); db[i] = { ...db[i], ...patch }; return db[i]; },
  async remove(id) { await delay(150); db = db.filter(t => t._id !== id); },
};
